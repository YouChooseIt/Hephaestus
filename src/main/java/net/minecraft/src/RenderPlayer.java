...
public class RenderPlayer extends RenderLiving {
    ...
    protected void renderName(EntityPlayer entityplayer, double d, double d1, double d2) {
        if (Minecraft.isGuiEnabled() && entityplayer != renderManager.livingPlayer) {
            ...
            final RenderLivingLabelEvent renderLivingLabelEvent = new RenderLivingLabelEvent(entityplayer, renderManager, d, d1, d2);
            Hephaestus.getInstance().getEventBus().post(renderLivingLabelEvent);

            if (renderLivingLabelEvent.isCancelled()) {
                return;
            }
